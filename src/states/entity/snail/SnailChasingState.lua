...
elseif self.player.x < self.snail.x then
	...
	if (tileBottomLeft and tileLeft) and (not tileBottomLeft:collidable() or tileLeft:collidable()) then
		self.snail.x = self.snail.x + SNAIL_MOVE_SPEED * dt
	end
else
	...
	if (tileBottomRight and tileRight) and (not tileBottomRight:collidable() or tileRight:collidable()) then
		self.snail.x = self.snail.x - SNAIL_MOVE_SPEED * dt
	end
end
end